...
const Index = () => {
    ...
    useEffect(() => {
        db.collection('scenes').where('owner', '==', 'Javier')
        ...
        db.collection('scenes').doc('QgvE68Mroa9ypamgq6bG')
        .collection('frames').orderBy('position').get().then((snap) => {
            snap.forEach((doc) => {
                console.log(`---------${doc.id} => `, doc.data())
            });
        });
    ...
    return (
        <Layout>
            <Helmet>
                ...
                <meta name = "description" value = "Text" />
            </Helmet>
